...
GET   /local/channels/:channelId/working-folders
PUT   /local/channels/:channelId/working-folders
...
POST  /local/channels/:channelId/agents/:identityId/open-diagnostic
...
Use `pnpm dev:review` or its explicit alias `pnpm app:review` for the disposable seeded simulation. Review mode coordinates temporary Channels and Relay storage, authenticated local control, a real `ChannelRuntimeRelay`, and Vite. It seeds `@you`, `@builder`, and `@reviewer`, sample messages, a simulated builder binding, and an unbound reviewer. The `review-mode` Runtime response is deterministic—not live model execution—and Ctrl-C removes its temporary data. `pnpm web:dev` remains frontend-only.
