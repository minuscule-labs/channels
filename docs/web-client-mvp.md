...
**Status:** Reviewable collaboration, participant creation, private configuration, Conversation administration, and explicit managed-agent lifecycle UI

## Goal

Provide a responsive local-first human collaboration client for discovering Workspaces and Conversations, reading live conversations, mentioning agents, and inspecting the public roster. The web app is a client, not a Runtime or orchestration layer.
...
```text
Browser ⇄ Conversations HTTP/SSE ⇄ internal agent host ⇄ MinuRuntime sessions
```

Normal work, agent responses, safe status projections, and explicit operational intent should flow through Conversations. The current direct localhost status overlay is transitional; private Workspace agent configuration, Runtime bindings, credentials, roots, leases, and diagnostics remain internal agent-host data rather than public Conversation data.
...
- Focused Workspace source dialog plus a dedicated Workspace Agents route. The Agents page combines readable/editable Workspace-local handle, public role, and delegation guidance with authenticated private Runtime, model, reasoning, instructions, skills, and status administration; it also shows Conversation assignments and private binding counts.
- Owner/admin participant creation inside Manage Conversation participants with human, agent, or service type; suggested/paste-safe handle; display name; public role; and inline Runtime/persona setup for execution identities.
...
The current localhost boundary is implemented as `@minu/channels-control`. It provides browser-safe contracts/client exports, a Node server facade over structural Conversation, binding, and Runtime status ports, and the current daemon/launcher. Under the accepted product direction this is a transitional internal agent-host package, not an independent product or alternate work path. Conversations core and the browser may not import agent-host, Runtime, daemon, or server modules.
...
GET   /local/conversations/:id/agents
POST  /local/conversations/:id/agents/:identityId/start
POST  /local/conversations/:id/agents/:identityId/replace
POST  /local/conversations/:id/agents/:identityId/stop
...
The real daemon now requires a browser session. A 256-bit one-time code valid for 60 seconds is redeemed at a loopback bootstrap endpoint for a separate random, HttpOnly, SameSite=Strict `/local` cookie bound to the configured stable human identity. `GET /local/session` returns only that public identity id. The composer verifies active human Conversation participation, keys drafts to that identity, and always uses it as the message author even if stale author-selection local storage is present. The credential is never placed in a URL, remains only in daemon memory, expires after eight hours, and is revoked by daemon restart. Browser and control use the same loopback hostname. This prevents accidental browser impersonation but is not public API authorization. Sanitized audit events record session issuance and rejection without recording either secret or private execution metadata. Playwright proves binding across refresh, authorship despite stale impersonation state, cookie bootstrap, and Vite `/local` proxy.

Private configuration writes and their administration UI are implemented. Agent administration lives at `/app/workspaces/:workspaceId/agents`; general Workspace settings retain only source configuration, while Conversation-specific Start/New session/Stop remains in each Conversation. Workspace source remains write-only, while the agent detail form prefills saved inline instructions and Runtime launch selections through authenticated local control. Active owners/admins create participants inside **Manage Conversation participants**, where the new identity is added to the Workspace and selected for the current Conversation in one flow. Display names suggest a handle, pasted leading `@` characters are normalized, and agent/service creation includes a required Runtime preference plus optional write-only persona. If restricted configuration fails after public identity/member creation, the UI reports that the participant was added and needs configuration rather than retrying into a duplicate identity. These collaboration writes still use the unauthenticated public Conversations API, so local browser gating is advisory until hosted authentication exists. Runtime start, replace, and stop derive their actor from the authenticated browser session; responses remain redacted and audit events contain no root, persona, adapter, or session values. The replace operation is presented as **New session** because it creates a new empty Runtime transcript rather than replacing the agent identity or Conversation history. New session is unavailable during active Conversation work, starts from current configuration, generation-fences the old binding, and discards pre-replacement pending work. Stop confirms that side effects remain, fences output before process termination, and presents the binding as stopped/disabled. Reconnect and interruption are enabled only through authenticated, audited, generation-fenced local control. General steering, interactive takeover, and control-plane identity creation remain disabled until their command-specific authorization and safety contracts are implemented.

## Deferred

- Authentication and hosted deployment. Do not add a client-only login facade while Conversations requests remain unauthenticated. When server authentication is introduced, reuse the MinuNotes Better Auth email-OTP/session pattern and bind the authenticated account to a Conversations human identity.
- Runtime steering, interruption, and reconnect controls beyond the implemented start/replace/stop operations.
- Separately named launch profiles shared across agents, arbitrary provider-option schemas, and optional Conversation working scopes beyond the implemented per-Workspace-agent model/reasoning profile; see [`execution-configuration.md`](execution-configuration.md).
- Reusing an existing global identity in another Workspace; Workspace member editing, disablement, role changes, and hosted authorization beyond the implemented Conversation-focused creation flow.
- Threads, reactions, attachments, search, unread state, and notifications.
- TUI parity.

## Collaboration hardening

The implemented hardening slice builds OpenCode-style client/service seams with T3 Code-style React interaction patterns without changing the Conversation domain model:
...
- Draft persistence isolated by Workspace, Conversation, and human author.
...
- Bounded Conversation history in every triggered turn, including older context that precedes the agent binding cursor; the cursor suppresses replay but does not hide conversation context.
- Playwright coverage against a real seeded Conversations server for message send, Workspace participant creation, named Conversation creation, optimistic participant replacement, historical attribution, Runtime start/replace/stop confirmations, roster revision, disconnect catch-up, stable retry keys, and accessible mobile drawers.
